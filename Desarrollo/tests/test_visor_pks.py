...
class ViewerPkTests(unittest.TestCase):
    def test_intervals_include_low_zoom_and_twenty_km(self) -> None:
        self.assertEqual(VALID_INTERVALS, {1, 5, 10, 20, 50, 100})
    ...
    def test_pk_intervals_filter_twenty_and_hundred(self) -> None:
        pks = gpd.GeoDataFrame({"MATRICULA": ["A-1", "A-1", "A-1"], "VALORKM": [20, 40, 100], "rotacion": [0, 0, 0]}, geometry=[Point(0, 0), Point(1000, 0), Point(2000, 0)], crs="EPSG:25830")
        lines = gpd.GeoDataFrame(geometry=[LineString([(0, 0), (2500, 0)])], crs="EPSG:25830")
        self.assertEqual([item["pk"] for item in pk_bbox_items(pks, PK_COLS, lines, ["A-1"], 20)], [20.0, 40.0, 100.0])
        self.assertEqual([item["pk"] for item in pk_bbox_items(pks, PK_COLS, lines, ["A-1"], 100)], [100.0])
    ...
    def test_frontend_uses_identified_roads_checkbox_and_popover(self) -> None:
        ...
        self.assertIn('interval: 20', script)
        self.assertIn('interval: 100', script)
